...
export default function PreQuizLoader({ format, onFinish }: PreQuizLoaderProps) {
    ...
    useEffect(() => {
        const fetchFacts = async () => {
            try {
                // ✅ FIX: Changed 'format' to 'context' and handle the result properly
                const result = await generateCricketFacts({ 
                    context: format, 
                    count: 5 
                });
                
                // ✅ FIX: Extract facts array from result
                const factsArray = Array.isArray(result)
                    ? result
                    : result.facts?.map((f: any) => f.fact) || getFallbackFacts();
                
                setFacts(factsArray.length > 0 ? factsArray : getFallbackFacts());
            } catch (error) {

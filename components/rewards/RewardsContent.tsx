...
interface RewardAd extends AdData {
  attemptId: string; // unique per quiz attempt
  format: string;
}
...
const ScratchCard = memo(
  ({
    ...
    // IMPORTANT: card UI reads ONLY its own isScratched prop
    return (
...
function RewardsContentComponent() {
  ...
  // Fetch ad for EVERY attempt (keeps duplicates)
  useEffect(() => {
  ...
  // Load scratched state per attempt
  useEffect(() => {
    ...
    rewardAds.forEach((ad) => {
      ...
      if (saved === 'true') initial[ad.attemptId] = true;
    });
  ...
  const handleScratch = (attemptId: string) => {
    setScratchedCards((prev) => ({ ...prev, [attemptId]: true }));
          ...
          <CarouselContent className="-ml-2">
            {rewardAds.map((ad) => (
              ...
              >
                <ScratchCard
                  ad={ad}
                  isScratched={scratchedCards[ad.attemptId] || false}

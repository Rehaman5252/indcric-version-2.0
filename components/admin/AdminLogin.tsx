...
import { AdminRole, ADMIN_ROLES, AdminSession } from '@/lib/admin-roles';
import { saveSessionToStorage } from '@/lib/admin-auth';
...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (email.toLowerCase() === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      // ✅ FIX: Use proper AdminSession type with AdminRole
      const adminSession: AdminSession = {
        ...
        phone: '+91-XXXXXXXXXX',
        role: 'superadmin' as const, // ✅ Type assertion to AdminRole
        permissions: ['*'],
        modules: ['all'],
      ...
      saveSessionToStorage(adminSession);
    ...
    try {
      ...
      if (result.user.email?.toLowerCase() === ADMIN_EMAIL) {
        ...
        const adminSession: AdminSession = {
          ...
          username: 'admin',
          displayName: result.user.displayName || 'Rehaman Syed',
          phone: result.user.phoneNumber || '+91-XXXXXXXXXX',
          role: 'superadmin' as const, // ✅ Type assertion to AdminRole
          permissions: ['*'],
          modules: ['all'],
          loginTime: new Date().toISOString(),
